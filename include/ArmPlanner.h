...
public:
	void compute(std::vector<VectorXd>& arm_trajectory);
	void initModel();
	void initializeData(VectorXd q_init_, VectorXd q_goal_);
	Eigen::VectorXd interpolate(const std::vector<Eigen::VectorXd> &data, double t, double dt, double t0, double tf);
	void updateValvePose(Eigen::Affine3d pose_, Vector3d dim_){
		obs_valve_.dim_ = dim_;
		obs_valve_.pose_ = pose_;
	}

	Eigen::VectorXd generateRandomGoal();
...
// variable Setting
	...
	std::vector<VectorXd> output_arm_trajectory_;
	std::vector<VectorXd> output_torso_trajectory_;
	VectorXd arm_trajectory_point_;
